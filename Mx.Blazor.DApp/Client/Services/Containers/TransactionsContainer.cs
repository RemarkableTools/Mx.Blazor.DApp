...
namespace Mx.Blazor.DApp.Client.Services.Containers
{
    public class TransactionsContainer
    {
        ...
        public TransactionsContainer(ISyncSessionStorageService sessionStorage)
        {
            ...
            var storedTxs = _sessionStorage.GetItem<List<StorageTx>>(BrowserStorage.TX_LIST);
            if (storedTxs != null)
            {
                ...
                if (storedTxs.Count > 0)
                {
                    ...
                    _sessionStorage.SetItem(BrowserStorage.TX_LIST, storedTxs);
                }
                else
                {
                    _sessionStorage.RemoveItem(BrowserStorage.TX_LIST);
                }
            }
        }
        ...
        public void NewTransaction(string title, string[] hashes)
        {
            ...
            var storedTxs = _sessionStorage.GetItem<List<StorageTx>>(BrowserStorage.TX_LIST) ?? new List<StorageTx>();
